...
function SideLink({ to, label, icon: Icon }: { to: string; label: string; icon: typeof Plus }) {
  return (
    <NavLink
      to={to}
      end={to === '/'}
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-full">
      {/* Desktop sidebar — the dark console rail */}
      <aside className="hidden w-60 shrink-0 flex-col bg-slate-900 md:flex">
        ...
        <div className="border-t border-slate-800 px-3 py-3">
          ...
          <div className="mt-2 flex items-center justify-between gap-2 px-3 py-1.5">
            ...
            <button onClick={() => void signOut()} className="text-slate-500 transition-colors hover:text-slate-200" aria-label="Sign out" title="Sign out">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex items-center justify-between bg-slate-900 px-4 py-3 md:hidden">
          <span className="font-display text-sm font-bold tracking-wide text-slate-100">CONTROL CENTRE</span>
          <div className="flex items-center gap-2">
            <button className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-700 text-white" aria-label="Quick add">
              <Plus className="h-5 w-5" />
            </button>
            <button onClick={() => void signOut()} className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400" aria-label="Sign out">
        ...
        <header className="hidden items-center justify-between border-b border-slate-200 bg-white px-6 py-4 md:flex">
          ...
          <button className="flex items-center gap-2 rounded-lg bg-slate-900 px-3.5 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-700">
